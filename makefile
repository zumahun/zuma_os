...
build:all
	rm zuma/boot/grub/ -r -f
	mkdir zuma/boot/grub/
	echo set default=0 >> zuma/boot/grub/grub/cfg
	echo set timeout=0 >> zuma/boot/grub/grub.cfg
	echo menuentry "zuma" { >> zuma/boot/grub/grub.cfg
	echo 		set root='(hd96)' >> zuma/boot/grub/grub.cfg
	echo 		multiboot /boot/kernel.bin >> zuma/boot/grub/grub.cfg
	echo } zuma/boot/grub/grub.cfg

	grub-mkrescue -o zuma.iso zuma/
